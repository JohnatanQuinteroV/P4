...
# ---
# 
# * Estudiante: **Johnatan Quintero Valverde**
# * Carné: **B65605**
# * Grupo: **2**
# 
# ---
...
#CREAMOS UNA FUNCION AUXILIAR MODIFICANDO FUNCION rgb_a_bit
#LA LLAMAMOS rgb_a_bit2
#Tendra dos columnas, un vector bidimensional
import numpy as np

def rgb_a_bit2(imagen):
    '''Convierte los pixeles de base 
    decimal (de 0 a 255) a binaria 
    (de 00000000 a 11111111).

    :param imagen: array de una imagen 
    :return: Un vector de (2 x k/2) bits 'int'
    '''
    # Obtener las dimensiones de la imagen
    x, y, z = imagen.shape
    ...
    bits = [format(pixel,'08b') for pixel in pixeles]
    bits_Rx = np.array(list(''.join(bits)))
    c= bits_Rx.reshape(-1,2) #ACA CAMBIAMOS DE UNIDIMENSIONAL A BIDIMENSIONAL
    
    return c.astype(int)
#POR EJEMPLO AL RECORRER EL RESULTADO LUCE ASI
#[1 0]
#[1 0]
#[0 0]
#[..]
...
#CREAMOS UNA FUNCION AUXILIAR MODIFICANDO FUNCION modulador
#LA LLAMAMOS modulador2
#tendra 2 portadoras y se deben sumar 
import numpy as np
...
def modulador2(bits, fc, mpp):
    ...
    # 2. Construyendo un periodo de las dos señales portadoras 
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp)
    portadora1 = np.cos(2*np.pi*fc*t_periodo) #portadora 1    
    portadora2 = np.sin(2*np.pi*fc*t_periodo) #portadora 2

    # 3. Inicializar la señal modulada s(t)
    t_simulacion = np.linspace(0, N*Tc, N*mpp) 
    senal_Tx1 = np.zeros(t_simulacion.shape)
    moduladora1 = np.zeros(t_simulacion.shape)  # señal de información
    senal_Tx2 = np.zeros(t_simulacion.shape)
    moduladora2 = np.zeros(t_simulacion.shape)  # señal de información
 
    # 4. Asignar las formas de onda según los bits (QPSK)
    #Como tenemos un vector bidimensional, entonces podemos asignar el bit 1 y bit 2 en el mismo for
    for i, bit in enumerate(bits):
        if bit[0] == 1:                  #el primer bit es de la portadora1
        ...
        if bit[1] == 1:                  #el segundo bit es de la portadora2
    ...
    # 5. Calcular la potencia promedio de la señal modulada
    Pm1 = (1 / (N*Tc)) * np.trapz(pow(senal_Tx1, 2), t_simulacion)
    Pm2 = (1 / (N*Tc)) * np.trapz(pow(senal_Tx2, 2), t_simulacion)
    
    senal_Tx3 = senal_Tx1 + senal_Tx2
    return senal_Tx3, Pm1, Pm2, portadora1, portadora2, moduladora1, moduladora2  
#note que el simbolo es de 2bits, osea 00,01,10,11


# In[20]:


#CREAMOS UNA FUNCION AUXILIAR MODIFICANDO FUNCION demodulador
#LA LLAMAMOS demodulador2
import numpy as np

def demodulador2(senal_Rx, portadora1, portadora2, mpp):
    '''Un método que simula un bloque demodulador
    ...
    # Dos vectores para bits obtenidos por la demodulación
    bits_Rx1 = np.zeros(N)
    bits_Rx2 = np.zeros(N)

    # Dos vectores para la señal demodulada
    senal_demodulada1 = np.zeros(M)
    senal_demodulada2 = np.zeros(M)

    # Energía de un período, para cada portadora
    Es1 = np.sum(portadora1**2)
    Es2 = np.sum(portadora2**2)

    # Demodulación
    for i in range(N):
        # Productos interno de dos funciones, usando ambas portadoras
        ...
        # Criterio de decisión por detección de energía, para ambas portadoras
        if Ep1 > Es1*0.8:
            bits_Rx1[i] = 1
        else:
            bits_Rx1[i] = 0
    ...
    return bits_Rx1.astype(int), bits_Rx2.astype(int), senal_demodulada1, senal_demodulada2


# In[32]:


#CREAMOS UNA FUNCION AUXILIAR MODIFICANDO FUNCION bits_a_rgb
#LA LLAMAMOS bits_a_rgb2
import numpy as np

def bits_a_rgb2(bits_Rx, dimensiones):
    '''Un blque que decodifica el los bits
    recuperados en el proceso de demodulación
    ...
    '''
    # Cantidad de bits
    bits_Rxx = bits_Rx.reshape(-1,1)  #hacemos un reshape para volver a una dimension
    N = len(bits_Rxx)
    # Se reconstruyen los canales RGB
    bits = np.split(bits_Rxx, N / 8)
...
#REALIZAMOS LA SIMULACION UTILIZANDO LAS NUEVAS FUNCIONES ASI COMO QPSK
import numpy as np
import matplotlib.pyplot as plt
import time
...
# 2. Codificar los pixeles de la imagen
bits_Tx = rgb_a_bit2(imagen_Tx)         #devuelve vector bidimensional

# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm1, Pm2, portadora1, portadora2, moduladora1, moduladora2 = modulador2(bits_Tx, fc, mpp)  #la señal senal_Tx ya tiene la suma de c1 y c2

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx1 = canal_ruidoso(senal_Tx, Pm, SNR)
senal_Rx2 = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx1, bits_Rx2, senal_demodulada1, senal_demodulada2 = demodulador2(senal_Rx, portadora1, portadora2, mpp)
# 6. Se visualiza la imagen recibida 
imagen_Rx1 = bits_a_rgb2(bits_Rx1, dimensiones)
Fig1 = plt.figure(figsize=(10,6))

imagen_Rx2 = bits_a_rgb2(bits_Rx2, dimensiones)
